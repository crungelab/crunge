...
class StructureTypePyRenderer(StructureTypeRenderer):
    def render(self):
        ...
        #self.out(f'py::class_<pywgpu::{class_name}>(m, "{class_name}")')
        '''
        PYSUBCLASS_BEGIN(_wgpu, pywgpu::AdapterPropertiesD3D, ChainedStructOut, AdapterPropertiesD3D)
        '''
        if node.chained:
    ...
    def render_init(self):
        ...
        for member in node.members:
            ...
            if member.length:
                #self.out << f'auto {member_name} = kwargs["{member_name}"].cast<py::array_t<{self.as_cppType(member_type.name)}>>();' << "\n"
                #self.out << f'auto {member_name} = kwargs["{member_name}"].cast<std::vector<{cppType}>>();' << "\n"
                ...
                if stripped_cppType == "char":
                    ...
                    self.out << f'obj.{member_cpp_name} = {member_name}.c_str();' << "\n"
                else:
                    self.out << f'auto {member_name} = kwargs["{member_name}"].cast<std::vector<{stripped_cppType}>>();' << "\n"
                    #self.out << f'obj.{member_cpp_name} = *{member_name}.data();' << "\n"
                    self.out << f'obj.{member_cpp_name} = {member_name}.data();' << "\n"
            else:
