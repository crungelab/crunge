...
from crunge.engine.builder.sprite import DefaultSpriteBuilder
...
class WyggleDna(Dna):
    def __init__(self, klass):
        ...
        self.tail_sprite = Sprite(self.create_texture(surface, name + 'tail', imgsize))

        #Eating
        self.draw_munchy_face(ctx)
        self.munchy_face_sprite = Sprite(self.create_texture(surface, name + 'munchy_face', imgsize))
        #Sad
        self.draw_happy_face(ctx, -1)
        self.sadFaceImage = Sprite(self.create_texture(surface, name + 'sadFace', imgsize))
        #Neutral
        self.draw_happy_face(ctx, 0)
        self.neutralFaceImage = Sprite(self.create_texture(surface, name + 'neutralFace', imgsize))
        #Happy
        self.draw_happy_face(ctx, 1)
        self.happy_face_sprite = Sprite(self.create_texture(surface, name + 'happy_face', imgsize))
        #
        self.face_sprite = self.happy_face_sprite
