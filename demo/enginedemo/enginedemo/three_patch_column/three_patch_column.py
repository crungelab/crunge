...
fn vs_main(@builtin(vertex_index) idx : u32) -> VertexOutput {
  ...
  // translate(x, y) * scale(width, height) and patches stack edge to edge.
...
class ThreePatchColumnDemo(Demo):
    image_names = [
    ...
    def create_pipeline(self):
        ...
        descriptor = wgpu.RenderPipelineDescriptor(
            label="Three Patch Column Pipeline",
    ...
    def layout_patches(self, x: float, y: float, width: float, height: float):
        """
        Return (y, height) for each patch, in top/middle/bottom order.

        Caps keep their aspect ratio at the column's width; the middle stretches.
        The projection is y-up, so (x, y) is the column's bottom-left corner and
        the top cap sits at the highest y.
        """
        top, middle, bottom = self.patches

        top_height = top.height * (width / top.width)
        bottom_height = bottom.height * (width / bottom.width)
        middle_height = max(height - top_height - bottom_height, 0.0)

        return [
            (y + bottom_height + middle_height, top_height),
            (y + bottom_height, middle_height),
            (y, bottom_height),
        ]
    ...
    def frame(self):
        ...
        column_width = 200.0
        column_height = viewport_height * (0.5 + 0.25 * math.sin(elapsed))

        column_x = (viewport_width - column_width) / 2
        column_y = (viewport_height - column_height) / 2

        placements = self.layout_patches(
            column_x, column_y, column_width, column_height
        )

        for patch, (patch_y, patch_height) in zip(self.patches, placements):
            model = glm.translate(glm.mat4(1.0), glm.vec3(column_x, patch_y, 0))
            model = glm.scale(model, glm.vec3(column_width, patch_height, 1))
