...
at their natural size; edges and center tile along their stretchable axes:

    TL  T  TR        T, B   repeat along x
    L   C  R         L, R   repeat along y
    BL  B  BR        C      repeats along both
...
class NinePatch:
    def __init__(
        ...
        thickness and the size of one repeat.
        """
        self.queue = queue
        self.scale = scale
    ...
    def set_rect(
        ...
        for i, (rect, region) in enumerate(zip(rects, self.slices)):
            ...
            # Middle column (T, C, B) repeats along x; middle row (L, C, R) along y
            tile_x = dest_w / (src_w * self.scale) if column == 1 and src_w > 0 else 1.0
            tile_y = dest_h / (src_h * self.scale) if row == 1 and src_h > 0 else 1.0
