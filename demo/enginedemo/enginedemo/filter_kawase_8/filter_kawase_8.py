...
fn fs_kawase(in: VSQuadOut) -> @location(0) vec4f {
  // If you already flipped V in vs_fullscreen, use in.uv directly.
  let uv = in.uv;

  let o = u.offset * u.texel_size; // convert pixel offset -> UV offset

  // 8 taps: 4 axis + 4 diagonal
  var c = vec4f(0.0);

  // Axis
  c += textureSample(src_tex, src_samp, uv + vec2f( o.x, 0.0));
  c += textureSample(src_tex, src_samp, uv + vec2f(-o.x, 0.0));
  c += textureSample(src_tex, src_samp, uv + vec2f(0.0,  o.y));
  c += textureSample(src_tex, src_samp, uv + vec2f(0.0, -o.y));

  // Diagonal
  c += textureSample(src_tex, src_samp, uv + vec2f( o.x,  o.y));
  c += textureSample(src_tex, src_samp, uv + vec2f(-o.x,  o.y));
  c += textureSample(src_tex, src_samp, uv + vec2f( o.x, -o.y));
  c += textureSample(src_tex, src_samp, uv + vec2f(-o.x, -o.y));
