...
fn fs_main(input : VertexOutput) -> @location(0) vec4<f32> {
  ...
  //let reflection = brdf(light, surface);
  //let rgb = LO * surface.albedo * surface.ao + surface.emissive;
  //let rgb = reflection * surface.albedo * surface.ao + surface.emissive;
  //let rgb = reflection * lightColor * surface.albedo * surface.ao + surface.emissive;
  
  let ambient = surface.albedo * surface.ao;
  let rgb = reflection + ambient + surface.emissive;
  {% else %}
  //let rgb = albedo * lightColor * ao + emissive;
  let rgb = surface.albedo * lightColor * surface.ao;
  {% endif %}
