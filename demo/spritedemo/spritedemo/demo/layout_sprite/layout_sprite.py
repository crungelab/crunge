...
class LayoutDemo(Demo):
    ...
    def setup(self):
        ...
        crate_sprite = self.sprite = SpriteLoader().load(
            "${images}/boxCrate.png"
        )

        self.node = Control2D(model=crate_sprite).seat(SpriteVu())


        ship_sprite = self.sprite = SpriteLoader().load(
            "${images}/playerShip1_orange.png"
        )
        self.ship_size = glm.vec2(ship_sprite.size)

        # Two children, so the flex column is visible: the first lands at the
        # bubble's top-left, the second directly under it. Neither is given a
        # position.
        self.ships = [
            Control2D(model=ship_sprite).seat(SpriteVu()) for _ in range(2)
        ]
        for ship in self.ships:
            self.node.add_child(ship)
        ...
        '''
        for ship in self.ships:
            ship.layout.set_size(self.ship_size.x, self.ship_size.y)
        '''
    ...
    def _draw(self):
        ...
        # Children: sizing intent again, applied to every ship
        imgui.separator()
        imgui.text("Children")
        changed, ship_width = imgui.drag_float(
            "Ship Width", self.ship_size.x, SIZE_STEP
        )
        if changed:
            self.ship_size.x = max(ship_width, 0.0)
            if alive:
                for ship in self.ships:
                    ship.layout.set_width(self.ship_size.x)

        changed, ship_height = imgui.drag_float(
            "Ship Height", self.ship_size.y, SIZE_STEP
        )
        if changed:
            self.ship_size.y = max(ship_height, 0.0)
            if alive:
                for ship in self.ships:
                    ship.layout.set_height(self.ship_size.y)

        # Computed, not asked for. The bubble's unscaled_size should track the
        # Width/Height sliders; each ship's position is whatever yoga decided,
        # in the bubble's own centered space, so the first one sits at
        # (-bw/2 + sw/2, +bh/2 - sh/2) and the second one a ship-height lower.
        if alive:
            bubble_size = self.node.unscaled_size
            imgui.text(f"Bubble computed {bubble_size.x:.2f}, {bubble_size.y:.2f}")
            for i, ship in enumerate(self.ships):
                pos = ship.position
                imgui.text(f"Ship {i} local {pos.x:.2f}, {pos.y:.2f}")
