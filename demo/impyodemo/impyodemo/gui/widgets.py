...
class PyoScope(Drawable):
    ...
    def draw(self):
        ...
        implot.set_next_axes_limits(0,500,0,500)
        if implot.begin_plot("Scope Plot"):
            for data in self.data:
                _, data = zip(*data)
                implot.plot_line(letters.pop(), data, len(data))
            implot.end_plot()
        imgui.end()
...
class PyoSpectrum(Drawable):
    ...
    def draw(self):
        ...
        implot.set_next_axes_limits(0,500,0,500)
        if implot.begin_plot("Spectrum Plot"):
            for data in self.data:
                _, data = zip(*data)
                implot.plot_line(letters.pop(), data, len(data))
            implot.end_plot()
...
class TableView(PyoObjectView):
    ...
    def draw(self):
        ...
        letters = list(string.ascii_uppercase)
        imgui.begin(self.title)
        implot.set_next_axes_limits(0,500,0,200)
        if implot.begin_plot("Table View"):
            implot.plot_line(letters.pop(), samples, len(samples))
            implot.end_plot()
        imgui.end()
...
class TableGraph(PyoObjectGraph):
    ...
    def draw(self):
        letters = list(string.ascii_uppercase)
        imgui.begin(self.title)
        implot.set_next_axes_limits(0, self.xrange, self.yrange[0], self.yrange[1])
        if implot.begin_plot("Table Waveform"):
            xs, ys = zip(*self.pts)
            implot.plot_line(letters.pop(), xs, ys, len(self.pts))
            implot.end_plot()
        imgui.end()
