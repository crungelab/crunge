...
class MultirateProcessing(Page):
    """
    ...
    def do_setup(self):
        ...
        gui.ctrl(src1, [SLMapFreq(722)], title="Sine frequency")
        src2 = SfPlayer(str(self.resource_path / "snds" / "flute.aif"), loop=True)
        # Input source interpolation.
        src = Interp(src1, src2, 0)
        gui.ctrl(src, [SLMap(0, 1, "lin", "interp", 0)], title="Source: sine <=> flute")

        # Control for the drive parameter of the distortion.
        drv = Sig(0)
        gui.ctrl(drv, title="Drive")
        ...
        self.output = output = Interp(dist, updist.sig(), 0, mul=0.5)
        gui.ctrl(output, [SLMap(0, 1, "lin", "interp", 0)], title="Up Sampling: without <=> with")

        sp = gui.spectrum(output)
