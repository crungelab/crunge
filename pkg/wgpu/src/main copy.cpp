...
void init_main(py::module &_wgpu, Registry &registry) {
    ...
    */
    ...
    /*Instance.def("process_events", [](const pywgpu::Instance& self)
    {
    ...
    PYEXTEND_BEGIN(pywgpu::Adapter, Adapter)
    Adapter.def("get_properties",
        [](const pywgpu::Adapter& self) {
            pywgpu::AdapterProperties properties;
    ...
    PYEXTEND_SCOPED_ENUM_BEGIN(pywgpu::BufferUsage, BufferUsage)
        //TextureUsage.def(py::self | py::self); //Doesn't work
        BufferUsage.def("__or__", [](pywgpu::BufferUsage& a, pywgpu::BufferUsage& b) {
        return (pywgpu::BufferUsage)(a | b);
            }, py::is_operator());
    PYEXTEND_END

    PYEXTEND_SCOPED_ENUM_BEGIN(pywgpu::ShaderStage, ShaderStage)
        //TextureUsage.def(py::self | py::self); //Doesn't work
        ShaderStage.def("__or__", [](pywgpu::ShaderStage& a, pywgpu::ShaderStage& b) {
