...
def _Queue_on_submitted_work_done_sync(
    ...
    # 5) Kick off the async request
    future = self._on_submitted_work_done(callback_info)
...
wgpu.Queue.on_submitted_work_done_sync = _Queue_on_submitted_work_done_sync

async def _Queue_on_submitted_work_done_async(
    self: wgpu.Queue, instance: wgpu.Instance
) -> None:
    loop = asyncio.get_event_loop()
    py_future = loop.create_future()
    ...
    # 5) Kick off the async request
    wgpu_future = self._on_submitted_work_done(callback_info)

    instance._pending_futures.append(wgpu_future)

    return await py_future

wgpu.Queue.on_submitted_work_done_async = _Queue_on_submitted_work_done_async
