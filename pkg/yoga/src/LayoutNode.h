...
{
public:
  static LayoutNode *createDefault(void);
  static LayoutNode *createWithConfig(Config *config);

  static void destroy(LayoutNode *node);

public:
  static LayoutNode *fromYGNode(YGNodeRef nodeRef);
...
public: // Prevent accidental copy
  LayoutNode(LayoutNode const &) = delete;

  LayoutNode const &operator=(LayoutNode const &) = delete;

public:
  void reset(void);

public: // Style setters
  void copyStyle(LayoutNode const &other);
...
public: // Tree hierarchy inspectors
  ...
  LayoutNode *getParent(void);
  LayoutNode *getChild(unsigned index);
