...
class HelloWgpu:
    ...
    def __init__(self):
        mesh_path = Path(__file__).parent.parent / "resources" / "models" / "fuze.obj"
        ...
        uv_coords = self.uv_coords = mesh.visual.uv.astype(np.float32)
        logger.debug(f'uv_coords type: {type(vertices)}')
        logger.debug(f'uv_coords:  {uv_coords}')
        n_uv_coords = self.n_uv_coords = len(vertices)
    ...
    def render(self, view: wgpu.TextureView):
        ...
        pass_enc.draw_indexed(self.n_indices)
        pass_enc.end()
