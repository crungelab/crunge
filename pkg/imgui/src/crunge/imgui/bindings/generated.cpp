...
void init_generated(py::module &_imgui, Registry &registry) {
    ...
    PYCLASS_BEGIN(_imgui, ImGuiIO, IO)
        ...
        IO.def_property("ini_filename",
            ...
            [](ImGuiIO& self, std::string source){ char* c = (char *)malloc(source.size() + 1); strcpy(c, source.c_str()); self.IniFilename = c; }
        );
        IO.def_property("log_filename",
            [](const ImGuiIO& self){ return self.LogFilename; },
            [](ImGuiIO& self, std::string source){ char* c = (char *)malloc(source.size() + 1); strcpy(c, source.c_str()); self.LogFilename = c; }
        );
        ...
        IO.def_property("backend_platform_name",
            ...
            [](ImGuiIO& self, std::string source){ char* c = (char *)malloc(source.size() + 1); strcpy(c, source.c_str()); self.BackendPlatformName = c; }
        );
        IO.def_property("backend_renderer_name",
            [](const ImGuiIO& self){ return self.BackendRendererName; },
            [](ImGuiIO& self, std::string source){ char* c = (char *)malloc(source.size() + 1); strcpy(c, source.c_str()); self.BackendRendererName = c; }
        );
    ...
    PYCLASS_BEGIN(_imgui, ImGuiInputTextCallbackData, InputTextCallbackData)
        ...
        InputTextCallbackData.def_property("buf",
            ...
            [](ImGuiInputTextCallbackData& self, std::string source){ char* c = (char *)malloc(source.size() + 1); strcpy(c, source.c_str()); self.Buf = c; }
        );
