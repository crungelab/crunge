...
from .rule_kit import Rule, RuleKit, Match
...
class Task[T: Task](BaseNode[T]):
    ...
    def __init__(self, action=None, msg=None, **bindings):
        super().__init__()
        if action:
            self.use(action)
        self.msg = msg
    ...
    def dispatch(self, msg: Message) -> bool:
        ...
        for match in self.match_rules(msg):
            ...
            #print("Dispatching message: {}".format(msg))
            #print("Fire: {} bindings={!r}".format(match, match.bindings))
            self.fire(match)
            fired = True
        if fired:
            return True
    ...
    def fire(self, match: Match) -> "Task[Any]":
        """Run a matched rule's action as a task of its own.

        The action is bound to us, not to the wrapper task, so `self` in a
        rule body is the node whose class declared the rule. Bindings from
        the trigger arrive as keywords, so `@rule(... _x_)` reads as
        `async def howdy(self, msg, x)`.
        """
        #print("Creating task for match: {} with bindings={!r}".format(match, match.bindings))
        task = Task(match.rule.bind(self), match.msg, **match.bindings)
