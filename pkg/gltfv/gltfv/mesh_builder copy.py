...
class MeshBuilder(ModelBuilder):
    mesh: Mesh = None
    ...
    def build(self, tf_mesh: gltf.Node):
        logger.debug("Building mesh")
        mesh: Mesh = self.mesh
        logger.debug(tf_mesh.__dict__)

        # Vertices
        vertices = tf_mesh.vertices.astype(np.float32)
        self.vertex_table.add_column(PosColumn('pos', vertices))

        # Normals
        normals = tf_mesh.vertex_normals.astype(np.float32)
        self.vertex_table.add_column(NormalColumn('normal', normals))

        # Visuals
        visual = tf_mesh.visual
        ...
        # Indices
        indices = tf_mesh.faces.astype(np.uint32)
        ...
        if visual_cls == tm.visual.TextureVisuals:
            tf_material = visual.material
            logger.debug(tf_material.__dict__)

            material = MaterialBuilder().build(tf_material)
