...
class TextureBuilderBase(ResourceBuilder[T_Resource]):
    def __init__(self, kit: TextureKit = ResourceManager().texture_kit, image_builder=ImageBuilder()) -> None:
        super().__init__(kit)
    ...
    def load_wgpu_texture(self, paths: List[Path]) -> wgpu.Texture:
        ...
        for path in paths:
            ...
            image = self.image_builder.load(path)
            images.append(image)
