...
class CollidableSpriteBuilder(SpriteBuilder):
    ...
    def build(self, texture: ImageTexture, rect: Rect2i = None, sampler: Sampler = None, color=colors.WHITE) -> Sprite:
        ...
        logger.debug(f"collision_rect: {collision_rect}")

        # in CollidableSpriteBuilder.build, after collision_rect is known
        full_center = glm.vec2(width, height) / 2.0
        crop_center = glm.vec2(bx + bw / 2.0, by + bh / 2.0)
        pixel_offset = crop_center - full_center
        origin_offset = glm.vec2(pixel_offset.x, -pixel_offset.y) / ppu   # scene units, Y-up
        logger.debug(f"origin_offset: {origin_offset}")

        # --- 10) Recenter & flip Y to your sprite's coordinate convention ---
        points = contour.reshape(-1, 2).astype(np.float32)

        points[:, 0] -= (bx + bw / 2.0) # recenter X
        points[:, 1] -= (by + bh / 2.0) # recenter Y
        ...
        #return Sprite(texture, rect, sampler, color, points, collision_rect)
        #return Sprite(texture, collision_rect, sampler, color, points, origin_offset=origin_offset)
        return Sprite(texture, rect, sampler, color, points, collision_rect)
    '''