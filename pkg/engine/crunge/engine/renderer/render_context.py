...
class RenderContext(Generic[T_Renderer]):
    def __init__(self, renderer: T_Renderer, root: RenderPlan[T_Renderer]) -> None:
        self.renderer = renderer
        self._stack: list[RenderPlan[T_Renderer]] = [root]

    def push_group(self, group: RenderPlan[T_Renderer]) -> None:
        self._stack.append(group)

    def pop_group(self) -> None:
        self._stack.pop()

    def current_group(self) -> RenderPlan[T_Renderer]:
