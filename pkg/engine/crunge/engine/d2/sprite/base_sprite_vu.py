...
class BaseSpriteVu[SpriteT: BaseSprite](Vu2D):
    ...
    def __init__(self, sprite: SpriteT = None) -> None:
        super().__init__()
        ...
        self._sprite: SpriteT = None
        self.sprite_membership: BaseSpriteMembership = None
        self.sprite = sprite

    # -- sprite ------------------------------------------------------------

    @property
    def sprite(self) -> SpriteT:
        return self._sprite

    @sprite.setter
    def sprite(self, sprite: SpriteT) -> None:
    ...
    def join(self) -> None:
        """(Re)join the group's model group. Tolerates either half being
    ...
    def build_uniform(self) -> NodeUniform:
        ...
        # The size the transform scales by, so a shader that lays out within
        # the node (nine patch) sees the same size the vertices were built from
    ...
    def _draw(self) -> None:
        ...
        self.bind(pass_enc)
        pass_enc.draw(4, self.instance_count)
