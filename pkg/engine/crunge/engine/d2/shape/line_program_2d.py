...
class LineRenderPipeline2D(RenderPipeline2D):

    @property
    def material_bind_group_layout(self):
        return BindGroupLayout(entries=[], label="Line Material Layout")
    ...
    def create_primitive_state(self):
        return wgpu.PrimitiveState(topology=wgpu.PrimitiveTopology.LINE_LIST)
...
class LineProgram2D(Program2D):
    def __init__(self):
        super().__init__()
        self.render_pipeline: LineRenderPipeline2D = None
    ...
    def create_render_pipeline(self):
        ...
        self.render_pipeline = LineRenderPipeline2D(
