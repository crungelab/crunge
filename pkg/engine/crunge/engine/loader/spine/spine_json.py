...
class SkinJSON(SpineBaseModel):
    name: str
    bones: list[str] = Field(default_factory=list)     # ASSUMPTION: unused until skin-scoped bones needed
...
class BoneTimelinesJSON(SpineBaseModel):
    ...
    shear: list[dict] = Field(default_factory=list)   # TODO: ShearKeyframe when we need it
