...
from .spine_json import (
    SpineSkeletonFile, BoneJSON, SlotJSON, SkinJSON,
    RegionAttachmentJSON, AnimationJSON,
)
from crunge.engine.d2.skeleton.skeleton_data import (
    SkeletonData, BoneData, SlotData, RegionAttachment,
)
from crunge.engine.d2.skeleton.animation import (
    Animation, RotateTimeline, TranslateTimeline, ScaleTimeline, AttachmentTimeline,
)
        ...
        for bone_name, bone_tl in anim.bones.items():
            ...
            if bone_tl.translate:
                timelines.append(TranslateTimeline(
                    bone_index=bone_index,
                    keyframes=[(kf.time, kf.x / ppu, kf.y / ppu, _parse_curve(kf.curve)) for kf in bone_tl.translate],
                ))
            if bone_tl.scale:
                timelines.append(ScaleTimeline(
                    bone_index=bone_index,
                    keyframes=[(kf.time, kf.x, kf.y, _parse_curve(kf.curve)) for kf in bone_tl.scale],
                ))

        for slot_name, slot_tl in anim.slots.items():
            slot_index = slot_index_by_name[slot_name]
            if slot_tl.attachment:
                timelines.append(AttachmentTimeline(
                    slot_index=slot_index,
                    keyframes=[(kf.time, kf.name) for kf in slot_tl.attachment],
                ))

        duration = max(
            (kf.time for tl_group in [anim.bones.values(), anim.slots.values()]
             for tl in tl_group
             for kfs in (getattr(tl, "rotate", []), getattr(tl, "translate", []),
                         getattr(tl, "scale", []), getattr(tl, "attachment", []))
             for kf in kfs),
            default=0.0,
        )
