...
void init_debug_draw_py(py::module &_box2d, Registry &registry) {
    ...
    */
    ...
    pyBase.attr("__init_subclass__") =
        classmethod(&PyDebugDrawBase::init_subclass,
                    // py::arg("cls"),
                    py::arg("kwargs") = py::kwargs()

        );

    // Also create a default cache on the base itself (so type(self) always has
    // _b2dd_cache, even if someone instantiates DebugDrawBase directly).
    {
        py::gil_scoped_acquire gil;
        PyDebugDrawBase::init_subclass(pyBase, py::kwargs());
    }

    // m.def("world_draw", &world_draw, py::arg("world_id"),
    // py::arg("debug_draw"));
}
