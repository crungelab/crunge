...
static void Body_SetPosition(b2BodyId bodyId, b2Vec2 position) {
    // Retrieve the body's current rotation to preserve it
    b2Rot currentRotation = b2Body_GetRotation(bodyId);

    // Teleport the body
    b2Body_SetTransform(bodyId, position, currentRotation);
}
