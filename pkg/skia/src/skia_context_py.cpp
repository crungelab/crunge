...
Context* CreateContext(pywgpu::Instance& _instance, pywgpu::Device& _device) {
    ...
    backendContext.fDevice = wgpu::Device(_device.Get());
   ...
   return fGraphiteContext.release();
}

void init_skia_context_py(py::module &_skia, Registry &registry) {
    _skia.def("create_context", &CreateContext);
}

/*
    void CreateSkiaContext() {
        skgpu::graphite::DawnBackendContext backendContext;
        backendContext.fInstance = wgpu::Instance(gfx().instance_.Get());
        backendContext.fDevice = gfx().device_;
        backendContext.fQueue = gfx().defaultQueue();
    
        //fDisplayParams = std::make_unique<skwindow::DisplayParams>();
        skwindow::GraphiteDisplayParamsBuilder paramsBuilder;
        fDisplayParams = paramsBuilder
            .colorType(kRGBA_8888_SkColorType)
            .msaaSampleCount(4)
            .disableVsync(true)
            .createProtectedNativeBackend(false)
